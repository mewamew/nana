...
class ConversationHistory:
    ...
    def __init__(self, max_turns: int = 20, llm_provider=None, embedding_provider=None, db=None):
        ...
        # 加载记忆提取 prompt 模板
        memory_prompt_path = os.path.join(os.path.dirname(__file__), "prompts", "memory_extract.md")
        with open(memory_prompt_path, "r", encoding="utf-8") as f:
            self.memory_extract_template = f.read()
        # 启动时从 SQLite 恢复对话
    ...
    async def _extract_memory(self, turns: list) -> dict:
        ...
        prompt = self.memory_extract_template.format(conversation_text=conversation_text)
